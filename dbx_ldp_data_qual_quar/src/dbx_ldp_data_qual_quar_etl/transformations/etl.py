...
rules = {
  "product_mapping": "(product_id IS NOT NULL)",
}
...
dp.create_streaming_table(name="products_deduped", comment="Deduped")

dp.create_auto_cdc_flow(
  target="products_deduped",  
...
@dp.table(
    name="final_data",
    comment="Final joined dataset"
)
@dp.expect_all(rules)
def final_data():
    orders = spark.readStream.table("LIVE.orders_bronze")
    products = spark.table("LIVE.products_deduped")

    joined_df = (
        orders.alias("A")
        .join(products.alias("B"), ["product_id"], "left")
        .selectExpr(
            "A.order_id",
            "A.customer_id",
            "A.order_date",
